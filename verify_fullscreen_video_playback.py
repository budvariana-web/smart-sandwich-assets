...
with sync_playwright() as p:
    ...
    page.wait_for_function("() => window.state && state.pages && state.pages.length === 24", timeout=30000)
    page.evaluate("() => { state.page = 2; renderPage(); }")
    page.wait_for_function("() => { const v = document.getElementById('fullscreen-video-el'); return v.readyState >= 2 && !v.paused && v.currentTime > 0; }", timeout=30000)
