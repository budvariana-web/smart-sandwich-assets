...
class Brochure(FPDF):
    ...
    def product_card(self, item: dict, image: Path | None, x: float, y: float, w: float = 91, h: float = 37) -> None:
        ...
        # Name. Extra vertical space prevents long English product names meeting the description.
        name_font = 8.3 if len(name) > 45 else 9.2
        desc_y = y + (21 if len(name) > 45 else 14)
        self.set_font("Arial", "B", name_font)
        self.set_text_color(*NAVY)
        self.set_xy(tx, y + 3)
...
def main() -> None:
    ...
    cover_items = [grouped["Burgers"][0], grouped["Sandwiches"][0], grouped["Appetizers"][0]]
    ...
    pdf.set_xy(18, 81)
    pdf.cell(90, 6, "burgers • sandwiches • Italian bites")
    pdf.set_font("Arial", "", 10)
    pdf.set_text_color(*WHITE)
    pdf.set_xy(18, 107)
    pdf.multi_cell(83, 5.2, "Freshly baked bread, signature sauces and bold flavours — prepared in the heart of Bar.")
    pdf.set_fill_color(*ORANGE)
    pdf.rect(17, 130, 72, 12, "F")
    pdf.set_font("Arial", "B", 9)
    pdf.set_text_color(*WHITE)
    pdf.set_xy(20, 133.5)
    pdf.cell(66, 4, "MENU & PRICES • 2026", align="C")
    ...
    pdf.set_xy(17, 278)
    pdf.cell(176, 4, "SHARE THIS MENU WITH FRIENDS — FLAVOUR IS BETTER TOGETHER", align="C")

    # --- PAGE 2: BURGERS + first sandwiches ---
    pdf.add_page()
    pdf.header_band("Burgers & sandwiches", "Fresh brioche, quality ingredients and signature sauces", 2)
    y = pdf.section_title("Burgers", 12, 35, "hot • hearty • prepared with character")
    for idx, item in enumerate(grouped["Burgers"]):
        x = 12 + (idx % 2) * 95
        if idx and idx % 2 == 0:
            y += 40
        pdf.product_card(item, images.get(item["name"]), x, y)
    y += 45
    y = pdf.section_title("Sandwiches", 12, y, "on ciabatta and focaccia")
    for idx, item in enumerate(grouped["Sandwiches"][:3]):
        x = 12 + (idx % 2) * 95
        yy = y + (idx // 2) * 40
    ...
    pdf.add_page()
    pdf.header_band("Sandwiches & Italian flavours", "Warm, crisp and generously filled", 3)
    y = pdf.section_title("Sandwiches", 12, 35, "continued")
    rest = grouped["Sandwiches"][3:]
    for idx, item in enumerate(rest):
        x = 12 + (idx % 2) * 95
        yy = y + (idx // 2) * 40
        pdf.product_card(item, images.get(item["name"]), x, yy)
    y += 80
    y = pdf.section_title("Bruschetta & focaccia", 12, y, "ideal with coffee or as a light bite")
    italian = grouped["Bruschetta"] + grouped["Focaccia"]
    ...
    pdf.add_page()
    pdf.header_band("Appetizers, sweets & extras", "Add something special to your order", 4)
    y = pdf.section_title("Appetizers", 12, 35, "golden, crisp, for every mood")
    for item in grouped["Appetizers"]:
        pdf.compact_row(item, images.get(item["name"]), 12, y)
        y += 21
    y += 3
    y = pdf.section_title("Bread & dessert", 12, y)
    for item in grouped["Bread"] + grouped["Desserts"]:
        pdf.compact_row(item, images.get(item["name"]), 12, y)
        y += 21
    y += 3
    y = pdf.section_title("Sauces", 12, y, "small extras — a big difference")
    # Condiments in a concise price grid so all products fit on the back page.
    sauces = grouped["Sauces"]
    ...
    pdf.set_text_color(*WHITE)
    pdf.set_xy(18, 261)
    pdf.cell(174, 5, "ORDER YOUR FAVOURITE FLAVOURS AT SMART SANDWICH BAR", align="C")
    pdf.set_font("Arial", "", 7.5)
    pdf.set_text_color(183, 221, 248)
    pdf.set_xy(18, 267)
    pdf.cell(174, 4, "Burgers • sandwiches • appetizers • coffee", align="C")
    pdf.footer()

    pdf.set_title("Smart Sandwich Bar — promotional brochure")
