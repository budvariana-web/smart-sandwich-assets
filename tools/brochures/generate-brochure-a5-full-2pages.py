...
OUT = ARTIFACTS / 'smart-sandwich-bar-brochure-a5-full-2-pages.pdf'
...
class PDF(FPDF):
    ...
    def footer(self):
        self.set_font('Glock','',4.5);self.set_text_color(*MUTED);self.set_xy(7,203);self.cell(134,2.5,'Bar, Crna Gora   •   @smartsandwichbar   •   Цены указаны в евро',align='C')
    ...
    def item(self,it,p,x,y,w,h):
        ...
        self.set_font('Glock','B',7.4 if len(name)<31 else 6.3);self.set_text_color(*INK);self.set_xy(tx,y+2);self.multi_cell(tw-13,3.05,name,align='L')
        name_bottom=self.get_y()
        self.set_font('Glock','B',7.7);self.set_text_color(*ORANGE);self.set_xy(x+w-12,y+2);self.cell(10,3,it['price'],align='R')
